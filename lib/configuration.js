...
function init() {
  ...
  try {
    ...
    logger.log("读取配置文件错误:", config_file, err);
  }
...
function init_ext(next) {
  ...
  try {
    ...
    try {
      ...
      logger.log('Load ext config fail, use default config', err, '< [', _model_nm, ']');
    } finally {
      next && next();
    }
...
function _save_to_file(next) {
  ...
  fs.writeFile(config.ext_config_file, txt, function(err) {
    logger.log(err || ('Save ext config ' + ext));
    next && next();
  });
}
